...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!q || q.trim().length < 2) {
      return res.status(400).json(errorResponse('Search query is required (minimum 2 characters)', 400));
    }
    ...
    const paginatedPlayers = paginate(rankedPlayers, page, searchLimit);
    const paginatedTeams = paginate(rankedTeams, page, searchLimit);
