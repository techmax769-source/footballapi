...
export default async function handler(req, res) {
  ...
  try {
    const { leagueId, countryId, teamId } = req.query;
    
    const cacheKey = getCacheKey('livescore', req.query);
    ...
    const data = await fetchFromAllSports('Livescore', params);
    
    setCache(cacheKey, data, 'livescore');
